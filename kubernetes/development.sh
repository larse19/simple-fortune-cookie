...
template2=`cat "frontend-deployment.yaml.template" | sed "s/{{NAME}}/$FRONTEND_NAME/g" | sed "s/{{BACKEND_NAME}}/$BACKEND_NAME/g"  | sed "s/{{IMAGE_NAME}}/$2/g"`
template3=`cat "backend-service.yaml.template" | sed "s/{{NAME}}/$BACKEND_NAME/g"`
template4=`cat "backend-deployment.yaml.template" | sed "s/{{NAME}}/$BACKEND_NAME/g" | sed "s/{{REDIS_NAME}}/$DB_NAME/g" | sed "s/{{IMAGE_NAME}}/$1/g"`
